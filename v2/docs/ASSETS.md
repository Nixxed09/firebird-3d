...
|---|---|---|
| `v2/assets/` | Original FIREBIRD kit, generated by `v2/tools/art/build_all.ps1` | 1st |
| `v2/assets/codex/` | Additional authored art | 2nd |
| `v2/assets/cc0/` | Legacy fallback art, with `CREDITS.md` | 3rd |

If two folders supply the same asset, the higher priority wins
(`ASSET_DIRS` in `src/render/assets.js`).

`node build.mjs` packs every manifest and a starter set into
`dist/assets-pack.js`, so a double-clicked `index.html` runs with authored
first-level art. Over HTTP, later models and textures load when a level or
weapon needs them. Use `--pack-all-assets` for a fully offline build. The
default pack is 7.30 MB of source files and the build warns over 8 MB.

## Art direction

**Dark hell-forge industrial.** Brick, soot, iron, embers and cyan holograms.
Chunky silhouettes and small emissive details stay readable in low light.
The game shows texture pixels crisp (nearest filtering) unless an entry
says `"filter": "linear"`.

- Embedded model textures: power of two, at most 1024 px. Surface sets: seamless 1024 px.
- Triangles: demons 3k–8k, first-person weapons 4k–10k, props 200–2k.
